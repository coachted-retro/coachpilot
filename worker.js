...
// Landing page at root, CoachPilot API on all other routes
...
export default {
  async fetch(request, env) {
    ...
    if ((path === '/' || path === '') && method === 'GET') {
      try {
        const page = await fetch('https://coachted-retro.github.io/coachpilot/index.html', {
          headers: { 'User-Agent': 'BeroFitWorker/1.0' }
        });
        ...
        return new Response('<h1>BeroFit Pro</h1><p>Coming soon. <a href="mailto:ted@berofitpro.com">Contact us</a></p>', {
    ...
    if (path === '/app' && method === 'GET') {
      try {
        ...
        return json({ error: 'App unavailable' }, 503);
      }
    }
    ...
    if (path === '/auth/verify' && method === 'GET') {
      ...
      await db.prepare(`UPDATE trainers SET session_token=?, token_expires=?, magic_token=NULL, magic_expires=NULL WHERE id=?`)
    ...
    if (path === '/auth/logout' && method === 'POST') {
      ...
      if (trainer) {
        await db.prepare('UPDATE trainers SET session_token=NULL, token_expires=NULL WHERE id=?').bind(trainer.id).run();
      }
    ...
    if (path === '/trainer/brand' && method === 'PUT') {
      ...
      await db.prepare(`UPDATE trainers SET brand_name=COALESCE(?,brand_name), brand_tagline=COALESCE(?,brand_tagline), brand_color=COALESCE(?,brand_color), brand_secondary=COALESCE(?,brand_secondary), brand_logo_url=COALESCE(?,brand_logo_url), brand_font=COALESCE(?,brand_font) WHERE id=?`)
        .bind(brand_name||null, brand_tagline||null, brand_color||null, brand_secondary||null, brand_logo_url||null, brand_font||null, trainer.id).run();
      return json({ success: true });
    }
    ...
    if (path === '/clients' && method === 'POST') {
      ...
      if (!trainer) return json({ error: 'Unauthorized' }, 401);
      const body = await request.json();
      const { name, email, phone, goal, start_date, notes, dob, gender, height, weight, conditions, glp1, is_ghost } = body;
      if (!name) return json({ error: 'Name required' }, 400);
      const id = genToken(16);
      await db.prepare(`INSERT INTO clients (id, trainer_id, name, email, phone, goal, start_date, notes, dob, gender, height, weight, conditions, glp1, is_ghost, status, created_at, check_in_streak) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
        .bind(id, trainer.id, name, email||null, phone||null, goal||null, start_date||null, notes||null, dob||null, gender||null, height||null, weight||null, conditions||null, glp1?1:0, is_ghost?1:0, 'active', Date.now(), 0).run();
    ...
    if (path.match(/^\/clients\/[^/]+$/) && method === 'GET') {
      ...
      const client = await db.prepare('SELECT * FROM clients WHERE id=? AND trainer_id=?').bind(id, trainer.id).first();
    ...
    if (path.match(/^\/clients\/[^/]+$/) && method === 'PUT') {
      ...
      if (!client) return json({ error: 'Not found' }, 404);
      const body = await request.json();
      const fields = ['name','email','phone','goal','start_date','notes','dob','gender','height','weight','conditions','glp1','status','emergency_contact','emergency_phone','medical_notes','photo_url'];
      const updates = []; const values = [];
      for (const f of fields) { if (body[f] !== undefined) { updates.push(`${f}=?`); values.push(body[f]); } }
      if (!updates.length) return json({ error: 'Nothing to update' }, 400);
      values.push(id, trainer.id);
      await db.prepare(`UPDATE clients SET ${updates.join(',')} WHERE id=? AND trainer_id=?`).bind(...values).run();
    ...
    if (path.match(/^\/clients\/[^/]+$/) && method === 'DELETE') {
      ...
      const client = await db.prepare('SELECT * FROM clients WHERE id=? AND trainer_id=?').bind(id, trainer.id).first();
      if (!client) return json({ error: 'Not found' }, 404);
      const sessions = await db.prepare('SELECT COUNT(*) as n FROM sessions WHERE client_id=?').bind(id).first();
      const checkins = await db.prepare('SELECT COUNT(*) as n FROM checkins WHERE client_id=?').bind(id).first();
      const confirm = url.searchParams.get('confirm');
      if (confirm !== 'DELETE') {
        return json({ warning: true, message: `You are about to permanently delete ${client.name} and all their data.`, sessions: sessions?.n || 0, checkins: checkins?.n || 0 });
      }
    ...
    if (path === '/sessions' && method === 'POST') {
      ...
      const client = await db.prepare('SELECT * FROM clients WHERE id=? AND trainer_id=?').bind(client_id, trainer.id).first();
      if (!client) return json({ error: 'Client not found' }, 404);
      const id = genToken(16);
      await db.prepare(`INSERT INTO sessions (id, client_id, trainer_id, session_date, session_time, duration_min, type, notes, status, recurring, recurring_freq, created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`)
        .bind(id, client_id, trainer.id, session_date, session_time||'09:00', duration_min||60, type||'training', notes||null, 'scheduled', recurring?1:0, recurring_freq||null, Date.now()).run();
    ...
    if (path.match(/^\/sessions\/[^/]+$/) && method === 'PUT') {
      ...
      if (!updates.length) return json({ error: 'Nothing to update' }, 400);
      values.push(id, trainer.id);
      await db.prepare(`UPDATE sessions SET ${updates.join(',')} WHERE id=? AND trainer_id=?`).bind(...values).run();
      return json({ success: true });
    }
    ...
    if (path.match(/^\/sessions\/[^/]+$/) && method === 'DELETE') {
      ...
      if (!trainer) return json({ error: 'Unauthorized' }, 401);
      const id = path.split('/')[2];
      const session = await db.prepare('SELECT * FROM sessions WHERE id=? AND trainer_id=?').bind(id, trainer.id).first();
    ...
    if (path === '/checkins' && method === 'POST') {
      ...
      const id = genToken(16);
      await db.prepare(`INSERT INTO checkins (id, client_id, trainer_id, weight, energy, sleep, nutrition_compliance, mood, notes, created_at) VALUES (?,?,?,?,?,?,?,?,?,?)`)
        .bind(id, client_id, trainer.id, weight||null, energy||null, sleep||null, nutrition_compliance||null, mood||null, notes||null, Date.now()).run();
      await db.prepare('UPDATE clients SET check_in_streak=check_in_streak+1, last_checkin=? WHERE id=?').bind(Date.now(), client_id).run();
      return json({ success: true, id }, 201);
    }
    ...
    if (path === '/dashboard' && method === 'GET') {
      ...
      const upcoming = await db.prepare(`SELECT s.*, c.name as client_name, c.photo_url FROM sessions s JOIN clients c ON s.client_id=c.id WHERE c.trainer_id=? AND s.session_date >= ? AND s.status='scheduled' ORDER BY s.session_date ASC, s.session_time ASC LIMIT 5`).bind(trainer.id, today).all();
      const needsAttention = await db.prepare(`SELECT * FROM clients WHERE trainer_id=? AND status='active' AND is_ghost=0 AND (last_checkin IS NULL OR last_checkin < ?) ORDER BY last_checkin ASC LIMIT 5`).bind(trainer.id, Date.now() - (7*24*60*60*1000)).all();
      return json({
